...
// Fetched when this page is opened, which is the only moment it can be drawn.
// A static import put recharts and victory-vendor - 565 KiB of it - into the
// server bundle of a route that is `ssr: false` and can never render there.
              ...
              {chartData.length === 0 ? (
                ...
                <Suspense fallback={null}>
                  <GradeChart
                    data={chartData}
                    rangeKey={range}
                    formatDay={(iso) => formatDay(iso, locale)}
                    formatNum={(n) => formatNum(n, locale)}
                  />
                </Suspense>
              )}
            </div>
